    ...
    <form method="post" action="{{ route('subject.store') }}" autocomplete="off">
      <div class="card-body">
        ...
        <div class="form-group{{ $errors->has('code') ? ' has-danger' : '' }}">
          <label>{{ __('Code') }}</label>
          <input type="text" name="code" id="code" class="form-control{{ $errors->has('code') ? ' is-invalid' : '' }}" placeholder="{{ __('Code') }}" value="">
          @include('alerts.feedback', ['field' => 'code'])
        </div>
        <div class="form-group{{ $errors->has('type') ? ' has-danger' : '' }}">
          <label>{{ __('Type') }}</label>
          <input type="text" name="type" id="type" class="form-control{{ $errors->has('type') ? ' is-invalid' : '' }}" placeholder="{{ __('Type') }}" value="">
    ...
    <div class="card ">
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table id="subject_table" class="table tablesorter">
                ...
                <td>
                  <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow" x-placement="bottom-end" style="position: absolute; transform: translate3d(-108px, 28px, 0px); top: 0px; left: 0px; will-change: transform;">
                      <button class="edit-modal dropdown-item" data-info="{{$item->id}},{{$item->name}},{{$item->code}},{{$item->type}},{{$item->is_active}}">Edit</a>
                    </div>
                  </div>
                </td>
              </tr>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form class="form-horizontal" role="form">
            ...
            <div class="form-group">
              <label class="control-label col-sm-2" for="mcode">Code</label>
              <div class="col-sm-10">
